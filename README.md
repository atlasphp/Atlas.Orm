...
I don't like code-generation, but I compromised on that in one limited way. It
turns out to be useful to have each table described as a PHP class, one that
just returns things like the table name, the column names, etc. That's the only
class that really gets generated by Atlas; the others are just empty extensions
of parent classes.  I guess nothing's perfect.
