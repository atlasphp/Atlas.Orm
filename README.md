...
- Atlas uses a base Record class, instead of plain-old PHP objects. If this were
a domain modelling system, a base class would be unacceptable. Because Atlas is
a *persistence* modelling system, I think a base class is less objectionable,
but for some people that's going to be a real problem.

Finally, Atlas supports **composite primary keys and composite foreign keys.**
Performance in these cases is sure to be slower, but it is actually supported.
...
If you want to change the mappings, use the `on()` method on the relationship. For example, if the threads table uses `author_id`, but the authors table uses just `id`, you can do this:
