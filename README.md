...
_Record_s and _RecordSet_s, does the trick. As you begin to need simple behaviors,
you can add them to the _Record_ and _RecordSet_ persistence model objects. (Rows
...
data source _Record_s and _RecordSet_s internally, but will not expose them. They
can manipulate the persistence model objects internally as much as they wish.
E.g., an Entity might have "getAddress()" and read from the internal _Record_
(which in turn reads from its internal _Row_ or _Related_ objects).  Alternatively,
...
of a project. Even so, the data-mapped _Row_, _Record_, and _RecordSet_ objects are
...
- Atlas uses base _Row_ and _Record_ classes, instead of plain-old PHP objects. If
...
Atlas is still in development and there are possibilities that the api may break.
...
If you are interested in playing with the latest bleeding edge branch, you can use the following:
...
in the `require-dev` section of `composer.json` in the root of project. This will
...
> N.b.: By default, Atlas uses generic _Record_ and _RecordSet_ classes for
> table data. You can create custom _Record_ and _RecordSet_ classes passing
> `--full` to `atlas-skeleton`; the Mapper will use the custom classes if
> available, and fall back to the generic ones if not. (Custom _Row_ classes are
...
// a more complex select of only the last 10 threads, with only some relationships
...
If you do not fetch a _Record_ "with" a relationship, it will be `null` in the
...
You can then address the _Record_'s underlying _Row_ columns and the relationship
...
Incidentally, you can also use the mapper to select non-_Record_ values directly
